...
| `trims [--session ID] [--json]` | 어떤 도구 출력을 언제, 어떤 근거로 정리했는지 보여 줍니다 |
...
- 정리한 출력은 `~/.codex/jev-runway/archive/`에 본인만 읽을 수 있도록 저장되며, 세션이 마지막으로 출력을 저장한 뒤 일주일이 지나면 삭제됩니다. 세션마다 남는 정리 기록(`trims.jsonl`)도 같은 위치에 저장되고 함께 삭제되며, 이 기록에는 정리된 호출의 입력이 앞의 200자까지 들어 있습니다.
...
- **정리가 일어난 뒤에 작업이 잘못되었을 때**: `jev-runway trims`로 세션을 찾은 다음, 그 ID나 Codex 세션 ID로 `jev-runway trims --session ID`를 실행하세요. 기록에는 도구의 전체 출력 없이 처음 요청이 나간 턴, 호출이 실행한 내용, Jev가 매긴 확률, 전체 출력이 저장된 위치가 항목마다 표시됩니다. 이 기록을 보면 모델이 스스로 실수한 것인지, 모델에게 아직 필요한 내용을 정리해서 생긴 문제인지 구분할 수 있습니다.
